...
/**
 * GOOGLE SHEETS CONNECTION & CREDENTIALS
 * Fetched from Script Properties for security.
 */
const scriptProps = PropertiesService.getScriptProperties();
const DATABASE_SHEET_ID = scriptProps.getProperty("DATABASE_SHEET_ID");
const PINCODE_SHEET_ID = scriptProps.getProperty("PINCODE_SHEET_ID");
...
// Add PINCODE_SHEET_ID to be ignored if it exists
if (PINCODE_SHEET_ID) {
  DATA_SYNC_CONFIG[PINCODE_SHEET_ID] = { type: "IGNORE" };
}
...
function handleLogin(params) {
  ...
  // Read from VERIFY sheet instead of USERS
  const verifyData = readFromSheet("VERIFY");
  let user = null;

  // Find user by username in the VERIFY sheet
  if (verifyData) {
    for (const key in verifyData) {
      const userData = verifyData[key];
      if (userData.USER && userData.USER.toString().toLowerCase() === username.toLowerCase()) {
        user = userData;
        break;
      }
    }
  ...
  if (user.PASS === hashedPassword) {
    ...
    // Update LASTLOGIN in the sheet
    user.LASTLOGIN = new Date().toISOString();
    const updatedVerifyData = verifyData;
    for (const key in updatedVerifyData) {
      if (updatedVerifyData[key].USER && updatedVerifyData[key].USER.toString().toLowerCase() === username.toLowerCase()) {
        updatedVerifyData[key] = user;
        break;
      }
    }
    writeToSheet("VERIFY", updatedVerifyData, "put");
...
function handleSendResetOtp(params) {
  ...
  // Find user by identifier in the VERIFY sheet
  if (verifyData) {
    for (const key in verifyData) {
      const userData = verifyData[key];
      if (userData.USER && userData.USER.toString().toLowerCase() === identifier.toLowerCase()) {
        user = userData;
        break;
      }
    }
...
function handleVerifyResetOtp(params) {
  ...
  const safeKey = sanitizeKey(identifier);
  // For updating a specific field in a sheet, we need to read the user, update the field, and write back
  const verifyData = readFromSheet("VERIFY");
  if (verifyData) {
    for (const key in verifyData) {
      if (verifyData[key].USER && verifyData[key].USER.toString().toLowerCase() === safeKey.toLowerCase()) {
        verifyData[key].TOKEN = resetToken; // Using TOKEN field instead of RESET_TOKEN
        writeToSheet("VERIFY", verifyData, "put");
        break;
      }
    }
  }
...
function handleResetPassword(params) {
  ...
  if (!user || user.TOKEN !== token) throw new Error("Invalid token.");

  // Update the user's password and clear the token
  const hashedPassword = hashString(newPassword);
  if (verifyData) {
    for (const key in verifyData) {
      if (verifyData[key].USER && verifyData[key].USER.toString().toLowerCase() === identifier.toLowerCase()) {
        verifyData[key].PASS = hashedPassword;
        verifyData[key].TOKEN = ""; // Clear the token field
        writeToSheet("VERIFY", verifyData, "put");
        break;
      }
    }
...
function handleInitiateRegistration(params) {
  ...
  // Check if user exists in VERIFY sheet
  const verifyData = readFromSheet("VERIFY");
  ...
  if (verifyData) {
    for (const key in verifyData) {
      const userData = verifyData[key];
      if (userData.USER && userData.USER.toString().toLowerCase() === regData.USER.toLowerCase()) {
        userExists = true;
        break;
      }
    }
  ...
  if (userExists) throw new Error("Username taken.");

  const safeEmailKey = sanitizeKey(regData.EMAIL);
  if (readFromSheet(`REGISTRATIONS/${safeEmailKey}`)) throw new Error("Already pending.");
...
function handleConfirmRegistration(params) {
  ...
  // For adding a new registration, we need to read existing data, add the new entry, and write back
  const regPath = `REGISTRATIONS`;
  let regData = readFromSheet(regPath);
  if (!regData) {
    regData = {};
  }
  regData[safeEmailKey] = cached.data;
  writeToSheet(regPath, regData, "put");
...
function handleFetchRegistrations(params, sessionUser) {
  ...
  const data = readFromSheet("REGISTRATIONS");
...
function handleDeclineRegistration(params, sessionUser) {
  ...
  // For deleting a registration, we need to read the registrations data, remove the entry, and write back
  const regPath = `REGISTRATIONS`;
  const regData = readFromSheet(regPath);
  if (regData) {
    delete regData[safeKey];
    writeToSheet(regPath, regData, "put");
  }
...
function handleFetchAllUsers(params, sessionUser) {
  checkAdminAccess(sessionUser);
  const data = readFromSheet("VERIFY");
...
function handleInitiateAddUser(params, sessionUser) {
  ...
  // Find admin user by username in the VERIFY sheet
  if (verifyData) {
    for (const key in verifyData) {
      const userData = verifyData[key];
      if (userData.USER && userData.USER.toString().toLowerCase() === sessionUser.username.toLowerCase()) {
        adminUser = userData;
        break;
      }
    }
  ...
  if (!adminUser || !adminUser.EMAIL) throw new Error("Admin email missing.");
...
function handleConfirmAddUser(params, sessionUser) {
  ...
  if (verifyData) {
    for (const key in verifyData) {
      const userData = verifyData[key];
      if (userData.USER && userData.USER.toString().toLowerCase() === newUser.USER.toLowerCase()) {
        userExists = true;
        break;
      }
    }
  ...
  // For adding a new user, we need to read existing users from VERIFY sheet, add the new user, and write back
  let verifyDataForUpdate = readFromSheet("VERIFY");
  if (!verifyDataForUpdate) {
    verifyDataForUpdate = {};
  }
  // Generate a unique key for the new user in the sheet
  const newKey = Utilities.getUuid();
  verifyDataForUpdate[newKey] = newUser;
  writeToSheet("VERIFY", verifyDataForUpdate, "put");
  if (newUser.EMAIL) {
    // For deleting a registration after user creation, we need to remove the entry from the REGISTRATIONS sheet
    const regPath = `REGISTRATIONS`;
    let regData = readFromSheet(regPath);
    if (regData && regData[sanitizeKey(newUser.EMAIL)]) {
      delete regData[sanitizeKey(newUser.EMAIL)];
      writeToSheet(regPath, regData, "put");
    }
  }
...
function handleInitiateAdminAccess(params, sessionUser) {
  ...
  // Find user by adminUsername in the VERIFY sheet
  if (verifyData) {
    for (const key in verifyData) {
      const userData = verifyData[key];
      if (userData.USER && userData.USER.toString().toLowerCase() === adminUsername.toLowerCase()) {
        user = userData;
        break;
      }
    }
...
function handleAdminUpdateUser(params, sessionUser) {
  ...
  // For updating user data, we need to read the current user data from VERIFY sheet, update the fields, and write back
  const verifyData = readFromSheet("VERIFY");
  if (verifyData) {
    for (const key in verifyData) {
      if (verifyData[key].USER && verifyData[key].USER.toString().toLowerCase() === params.targetUser.toLowerCase()) {
        Object.assign(verifyData[key], safeUpdates);
        writeToSheet("VERIFY", verifyData, "put");
        break;
      }
    }
...
function handleAdminDeleteUser(params, sessionUser) {
  checkAdminAccess(sessionUser);
  checkSudo(params.sessionID);
  // For deleting a user, we need to read the VERIFY data, remove the user, and write back
  const verifyData = readFromSheet("VERIFY");
  if (verifyData) {
    for (const key in verifyData) {
      if (verifyData[key].USER && verifyData[key].USER.toString().toLowerCase() === params.targetUser.toLowerCase()) {
...
function verifyAndFetchAppData(params, sessionUser) {
  ...
  for (const [path, config] of Object.entries(DATA_SYNC_CONFIG)) {
    ...
    let fetchedData = readFromSheet(path, query);
...
function handleToggleSystemStatus(params, sessionUser) {
    ...
    // Find master user by username in the VERIFY sheet
    if (verifyData) {
      for (const key in verifyData) {
        const userData = verifyData[key];
        if (userData.USER && userData.USER.toString().toLowerCase() === sessionUser.username.toLowerCase()) {
...
function validateSession(params) {
  ...
  const cachedSessionStr = sessionID ? CacheService.getScriptCache().get(sessionID) : null;
  if (!cachedSessionStr) throw new Error("Session expired.");
  return JSON.parse(cachedSessionStr);
}

function readFromSheet(path, queryString = "") {
  const sheet = SpreadsheetApp.openById(DATABASE_SHEET_ID);
  const sheetName = path.startsWith('/') ? path.substring(1) : path;
  const worksheet = sheet.getSheetByName(sheetName);
  ...
  // Convert 2D array to object format similar to Firebase
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    // Use first column value as key if available, otherwise use row index
    const key = row[0] !== undefined && row[0] !== "" ? row[0].toString() : `row_${i}`;
...
function writeToSheet(path, data, method = "put") {
  const sheet = SpreadsheetApp.openById(DATABASE_SHEET_ID);
  const sheetName = path.startsWith('/') ? path.substring(1) : path;
  let worksheet = sheet.getSheetByName(sheetName);
  ...
  if (!worksheet) {
    worksheet = sheet.insertSheet(sheetName);
  }
  ...
  // If data is empty, just return after clearing
  if (!data) return;

  // Determine headers from the first data object
  let headers = [];
  if (Array.isArray(data) && data.length > 0) {
    headers = Object.keys(data[0]);
  } else if (typeof data === 'object' && data !== null) {
    // If data is a single object, we need to handle differently
    // For now, assume we're updating specific rows based on keys
  ...
  if (headers.length > 0) {
    ...
    if (Array.isArray(data)) {
      for (let i = 0; i < data.length; i++) {
        ...
        worksheet.getRange(i + 2, 1, 1, headers.length).setValues([row]);
      }
    } else if (typeof data === 'object' && data !== null) {
      // Handle single object data
      const row = [];
      for (let j = 0; j < headers.length; j++) {
        row.push(data[headers[j]] || "");
      }
      worksheet.getRange(2, 1, 1, headers.length).setValues([row]);
    }
  }
}
...
function forceAuth() {
  ...
  try { SpreadsheetApp.openById(DATABASE_SHEET_ID); } catch(e) {} // Authorize Google Sheets
